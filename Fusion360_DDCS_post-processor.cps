...
properties = {
 ...
 // --- CUSTOM EDIT START: Safe Park & Hidden Options ---
  safePositionMethod: {
    title      : "Safe Retracts",
    description: "G54 Z60 safe retract (hardcoded for M350)",
    group      : "homePositions",
    type       : "string",
    value: "G54Z60",
    scope: "hidden"  // Hide it completely since we're hardcoding G54 Z60
  },
  homePositionEnd: {
  ...
  victoryDance: {
    ...
    group      : "preferences",
    type       : "boolean",
    value      : true, // Default to YES because you earned it
    scope      : "post"
  },
  askForDance: {
    title      : "Confirm Dance (M00)",
    description: "Pauses machine before dancing. Press START to dance, STOP to finish.",
    group      : "preferences",
    type       : "boolean",
    value      : true, // Default ON so you always have the choice
    scope      : "post"
  },
};
  // --- CUSTOM EDIT END ---
...
function onClose() {
 ...
 // --- 🕺 VICTORY DANCE (THE DIAMOND TORNADO) 🕺 ---
  if (getProperty("victoryDance")) {
    writeComment("-----------------------------------");
    writeComment("   JOB COMPLETE - DIAMOND DANCE    ");
    writeComment("-----------------------------------");
    
    // 1. Switch to Incremental (G91) - Save state conceptually by ensuring we return to G90 later
    ...
    if (getProperty("askForDance")) {
      writeComment(" READY TO DIAMOND? (Total Rise: " + (safeLift + 30) + "mm)");
      writeBlock(mFormat.format(0));
    }

    // 4. DANCE VARIABLES
    var maxRadius   = 20; // Widest point
    var phaseHeight = 15; // Height of EACH half
    var turnsPerPhase = 4;
    var feedRate    = 6000;
    var stepsPerTurn = 36; 
    var stepsPhase   = turnsPerPhase * stepsPerTurn;

    var virtX = 0;
    var virtY = 0;
    var virtZ = 0;

    // FIX #3: Use feedFormat.format() for safe output
    writeBlock(gMotionModal.format(1), "F" + feedFormat.format(feedRate));

    // --- PHASE 1: SPIRAL UP & OUT ---
    for (var i = 1; i <= stepsPhase; ++i) {
      var fraction = i / stepsPhase;
      var angle = fraction * turnsPerPhase * 6.28318;
      var r = fraction * maxRadius;
      
      var nextX = r * Math.cos(angle);
      var nextY = r * Math.sin(angle);
      var nextZ = fraction * phaseHeight;
      
      writeBlock(
        "X" + xyzFormat.format(nextX - virtX),
        "Y" + xyzFormat.format(nextY - virtY),
        "Z" + xyzFormat.format(nextZ - virtZ)
      );
      virtX = nextX; virtY = nextY; virtZ = nextZ;
    }

    // --- PHASE 2: SPIRAL UP & IN ---
    for (var i = 1; i <= stepsPhase; ++i) {
      var fraction = i / stepsPhase;
      var currentAngle = (turnsPerPhase * 6.28318) + (fraction * turnsPerPhase * 6.28318);
      var r = maxRadius * (1 - fraction);
      
      var nextX = r * Math.cos(currentAngle);
      var nextY = r * Math.sin(currentAngle);
      var nextZ = phaseHeight + (fraction * phaseHeight);
    ...
    // --- FINISH: DROP DOWN THE CENTER ---
    writeBlock(gMotionModal.format(0), "Z-" + xyzFormat.format(virtZ));
    
    // 5. Cleanup
    writeBlock(gAbsIncModal.format(90)); // Force Absolute Mode back
    gMotionModal.reset(); // FIX #1: Reset motion modal so next moves don't inherit the dance feed
  }

 // --- CUSTOM END PARKING LOGIC (DYNAMIC DDCSE MATH) ---
  
  // 1. Vertical Safety Retract (Machine Z-10)
  // Logic: Moves to Machine Z-10 by subtracting the active Z offset
  // #578 is the active WCS index. #802 is the base Z offset variable.
  // Formula: -10 - (Value of Z offset for current WCS)
  writeBlock(gMotionModal.format(0), "Z[-10 - #[802 + #578 * 5]]");

  // Reset internal state so Post knows we retracted
  ...
  // 2. Horizontal Park Move
  var endPos = getProperty("homePositionEnd");

  if (endPos == "safePark") {
    // Safe Park using User Vars #1153 (X) and #1154 (Y)
    // Formula: [Target Machine Pos] - [Active WCS Offset]
    // X Offset Base: #800 | Y Offset Base: #801
    writeBlock(
    ...
    // Standard G28 Home (Safe default)
    writeBlock(gAbsIncModal.format(91), gFormat.format(28), "X" + xyzFormat.format(0), "Y" + xyzFormat.format(0));
...
function onRapid(_x, _y, _z) {
  // Force XYZ output for M350 DDCS compatibility
  forceXYZ();
  ...
  if (retract && retract.words.length > 0) {
    ...
    for (var i in retract.words) {
     ...
     switch (retract.method) {
      ...
      case "G54Z60":
      default:
        // M350 DDCS: Use G54 Z60 for safe retract (60mm above spoilboard)
        forceModals(gMotionModal);
        writeBlock(gFormat.format(54), gMotionModal.format(0), "Z" + xyzFormat.format(60.0));
        state.retractedZ = true;
        break;
      }
      machineSimulation({
  ...
  writeStartBlocks(isRequired, function() {
    ...
    if (machineConfiguration.isHeadConfiguration()) { // head/head head/table kinematics
      ...
      if (currentSection.isMultiAxis() && getSetting("workPlaneMethod.prepositionWithTWP", true) && getSetting("workPlaneMethod.useTiltedWorkplane", false) &&
        ...
        machineSimulation({x:position.x, y:position.y, z:position.z});
        writeBlock(gMotionModal.format(motionCode.single), getOffsetCode(), hOffset, additionalCodes[1]);
